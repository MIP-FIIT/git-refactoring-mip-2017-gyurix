...
public final class SU {
    ...
    static void initOfflinePlayerManager() {
        try {
            ...
            entityPlayerC = entityPlayerClass.getConstructor(mcServerClass, worldServerClass, com.mojang.authlib.GameProfile.class, pIMClass);
    ...
    /**
     * Save files from the given plugins jar file to its subfolder in the plugins folder. The files
     * will only be saved if they doesn't exists in the plugins subfolder.
     *
    ...
    public static GameProfile getProfile(Player plr) {
        try {
            return (GameProfile) plr.getClass().getMethod("getProfile").invoke(plr);
        } catch (Throwable e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static Player loadPlayer(UUID uuid) {
        try {
            ...
            Player plr = (Player) getBukkitEntityM.invoke(entityPlayerC
                    .newInstance(mcServer, worldServer, new com.mojang.authlib.GameProfile(uuid, player

                            .getName()), playerInterractManagerC
    ...
    public static String itemToString(ItemStack in) {
        ...
        for (Map.Entry<Enchantment, Integer> ench : meta.getEnchants().entrySet())
            out.append(' ').append(ench.getKey().getName()).append(':').append(ench.getValue());
